...
extension GameNoteAddingEditingViewModelUnitTest: GameNoteAddingEditingViewModelDelegate {
    func didNoteLoaded(gameNote _: RAWGVideoGames.GameNote?, pageViewMode _: RAWGVideoGames.PageViewMode) {
        fetchExpectation.fulfill()
    }

    func didAddReminder(gameNote _: RAWGVideoGames.GameNote) {
        fetchExpectation.fulfill()
    }

    func didUpdateReminder(gameNote _: RAWGVideoGames.GameNote) {
        fetchExpectation.fulfill()
    }

    func didAddNote(gameNote _: RAWGVideoGames.GameNote) {
        fetchExpectation.fulfill()
    }

    func didUpdateNote(gameNote _: RAWGVideoGames.GameNote) {
        fetchExpectation.fulfill()
    }

    func didAuthErrorOccur(error _: String) {
        fetchExpectation.fulfill()
    }
}
