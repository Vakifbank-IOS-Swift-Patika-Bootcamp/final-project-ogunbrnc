...
final class GameNoteListViewController: BaseViewController {
    ...
    @IBOutlet private var notesRemindersSegmentedControl: UISegmentedControl!
    ...
    @objc private func didTapAddNote() {
        guard let noteAddingEditingViewController = storyboard?.instantiateViewController(withIdentifier: "GameNoteAddingEditingViewController") as? GameNoteAddingEditingViewController else {
...
extension GameNoteListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if notesRemindersSegmentedControl.selectedSegmentIndex == 0 {
            ...
            guard let cell = tableView.dequeueReusableCell(withIdentifier: GameNoteReminderTableViewCell.identifier, for: indexPath) as? GameNoteReminderTableViewCell,
                  let note = viewModel.getGameNoteHasReminder(at: indexPath.row)
            else {
                return UITableViewCell()
            }
            cell.configure(with: note)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard let noteAddingOrEditingViewController = storyboard?.instantiateViewController(withIdentifier: "GameNoteAddingEditingViewController") as? GameNoteAddingEditingViewController else {
            fatalError("View Controller not found")
        }
