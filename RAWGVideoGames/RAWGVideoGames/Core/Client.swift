...
final class Client {
    ...
    @discardableResult
    class func taskForGETRequest<ResponseType: Decodable>(url: URL, responseType _: ResponseType.Type, completion: @escaping (ResponseType?, Error?) -> Void) -> URLSessionDataTask {
    ...
    class func getGames(by orderString: String = "", with urlString: String, search searchString: String = "", completion: @escaping (Result<GetGamesResponseModel, Error>) -> Void) {
        ...
        taskForGETRequest(url: requestURL, responseType: GetGamesResponseModel.self) { responseModel, _ in
